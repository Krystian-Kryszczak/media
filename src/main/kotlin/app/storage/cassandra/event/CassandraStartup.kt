...
class CassandraStartup(private val cqlSession: CqlSession) {
    ...
    private fun createWatchTable() = execute(getMediaTableBase("watch").build())
    private fun createListenTable() = execute(getMediaTableBase("listen").build())
    private fun getMediaTableBase(name: String): CreateTable = getExhibitTableBase(name)
