...
abstract class LocalMediaStorage(
    ...
    private var storageDir: String = System.getenv("LOCAL_CLOUD_BLOB_STORAGE_DIR")!!, // ?: "./blob/",
): MediaStorage {
    constructor(mediaType: app.cloud.Cloud.MediaType) : this(mediaType.toString())
    ...
    override fun save(id: UUID, inputStream: InputStream, creatorId: UUID, private: Boolean): Completable = Completable.create { emitter ->
        ...
        if (!dir.mkdir()) {
            emitter.onError(RuntimeException("Container with id $id already exists!"))
        }
        ...
        if (!metadata.canWrite() && !metadata.setWritable(true)) {
            emitter.onError(RuntimeException("Cannot write to metadata file in $id container!"))
        }
